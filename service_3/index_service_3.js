...
const app = express();
const serviceName = 'sample-graphql-3';
const hostedURL = 'http://localhost:4200/graphql';
...
const resolvers = {
	Query: {
		sendEvent: async (parent, args) => {
			return axios
				.post('http://localhost:4600/send-event', {
					action: args.message,
				})
...
app.listen({ port: 4200 }, () => {
	console.log(`Server 3 is ready at http://localhost:4200`);
});
